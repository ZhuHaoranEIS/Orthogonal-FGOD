...
from .rotated_anchor_free_head_more import MORE_RotatedAnchorFreeHead
...
class RotatedORTHFCOSHead(MORE_RotatedAnchorFreeHead):
    ...
    def __init__(self,
                 ...
                 save_path='/home/xxx/orth_bank/FCOS_fair1mv2.npy',
                 loss_cls=dict(
                     type='FocalLoss',
                     use_sigmoid=True,
                     gamma=2.0,
    ...
    def forward_single(self, x, scale, stride):
        ...
        """
        bbox_pred, cls_feat, reg_feat, feats_ori = super().forward_single(x)
        image_feats = feats_ori / feats_ori.norm(dim=1, keepdim=True)
        text_feats = self.new_value.to(feats_ori.dtype).to(feats_ori.device)
        text_probs = 100 * torch.einsum('bcwh, nc->bnwh', image_feats, text_feats)
        ...
        return text_probs, bbox_pred, angle_pred, centerness, feats_ori
            

    @force_fp32(
        apply_to=('cls_scores', 'bbox_preds', 'angle_preds', 'centernesses', 'cls_feat'))
    def loss(self,
             cls_scores,
             bbox_preds,
             angle_preds,

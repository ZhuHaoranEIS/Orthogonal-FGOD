...
class RotatedORTHRetinaHead(RotatedAnchorHead):
    ...
    def __init__(self,
                 ...
                 conv_cfg=None,
                 norm_cfg=None,
                 save_path='/home/zhr/PETDet/mmrotate/models/roi_heads/bbox_heads/orth_bank/RETINA.npy',
        ...
        super(RotatedORTHRetinaHead, self).__init__(
            num_classes,
            in_channels,
        ...
        # for self.num_anchors
        self.new_value = self.new_value.unsqueeze(0).repeat(self.num_anchors, 1, 1)
        self.new_value = self.new_value.reshape(-1, self.feat_channels)
    ...
    def forward_single(self, x):
        ...
        H_feats, W_feats = cls_feat.shape[2:4]
        image_feats = cls_feat.view(cls_feat.shape[0], cls_feat.shape[1], -1).permute(0,2,1) # B * HW * 512
        image_feats = image_feats / image_feats.norm(dim=-1, keepdim=True)
        text_feats = self.new_value.to(cls_feat.dtype).to(cls_feat.device)
        text_probs = (100.0 * image_feats @ text_feats.T) # B * HW * 8
        text_probs = text_probs.permute(0,2,1).view(cls_feat.shape[0], self.num_anchors * self.cls_out_channels, H_feats, W_feats)
        
        # return cls_score, bbox_pred
        return text_probs, bbox_pred

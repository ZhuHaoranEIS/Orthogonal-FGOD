    ...
    def loss(self,
        ...
        if cls_score.numel() > 0:
            ...
            if pos_inds.any():
                ...
                pos_inds_opl = ((labels >= 0) & (labels < bg_class_ind)).nonzero().reshape(-1)
                if self.use_opl:
                    loss_opl = self.loss_opl(cls_feat[pos_inds_opl], labels[pos_inds], avg_factor=len(pos_inds))
                    losses['loss_opl'] = loss_opl
                else:
                    loss_opl = len(pos_inds)

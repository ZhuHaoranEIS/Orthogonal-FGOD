        ...
        for i in range(num_classes):
            ...
            for j in range(num_classes):
                if confusion_matrix[i, j] <= 1:
                    ...
                    all_number.append(round(confusion_matrix[i, j], 2))
            if sum(all_number) != 100:
                for j in range(num_classes):
                    index = -1 - j
                    if all_number[index] != 0:
                       all_number[index] = 100 - sum(all_number) + all_number[index]
                       break
            for j in range(num_classes):
                if int(confusion_matrix[i, j]) == 0:
                    continue
                ...
                ax.text(
                    ...
                    i,
                    '{:.2f}'.format(
                        float(all_number[j]) if not np.isnan(confusion_matrix[i, j]) else -1),
                    ha='center',
                    va='center',
                    color=color,
                    size=7)
...
def main():
    ...
    confusion_matrix = np.load('/home/zhr/work_dirs/rotated_fcos_orth/petdet_confusion_matrix_0.5.npy')
